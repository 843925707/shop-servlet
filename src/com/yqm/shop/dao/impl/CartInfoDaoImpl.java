...
public class CartInfoDaoImpl implements ICartInfoDao{
	...
	public int add(CartInfo cf) {
		DBHelper db = new DBHelper();
		String sql = "insert into cartinfo value(?, ?, ?, ?)";
	...
	public List<CartInfo> findByCnos(String[] cnos) {
		DBHelper db = new DBHelper();
		String sql = "select cno, c.gno, num, price, gname,pics, unit, weight from cartinfo c, goodsinfo g where c.gno=g.gno and cno in(";
		List<Object> params = new ArrayList<Object>();
		for(String cno : cnos){
			sql +="?,";
			params.add(cno);
